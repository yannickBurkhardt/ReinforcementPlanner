...
class Nav2DWorldEnv(gym.Env):
	...
	low = np.hstack((np.array([0, 0]), np.array([0, 0]), low_obs_p, low_obs_p, low_obs_size))
        ...
        if self.dynamic_obstacles:
            if config.ENV.CONSIDER_VELOCITIES:
                low = np.hstack((np.array([-2.0]*self.state_space_dim), np.array([-2.0, -2.0]), low_obs_p, low_obs_v, np.array([-self.max_vel]*2)))
                high = np.hstack((np.array([2.0]*self.state_space_dim), np.array([2.0, 2.0]), high_obs_p, high_obs_v, np.array([-self.max_vel]*2)))
            else:
                low = np.hstack((np.array([-2.0]*self.state_space_dim), np.array([-2.0, -2.0]), low_obs_p, np.array([-self.max_vel]*2)))
                high = np.hstack((np.array([2.0]*self.state_space_dim), np.array([2.0, 2.0]), high_obs_p, np.array([-self.max_vel]*2)))
    ...
    def step(self, action):
        ...
        if config.ENV.MOTION_MODEL == "holonomic":
            ...
            self._agent_location[2] += action[1]*10.0 #TODO: check how to limit rotation velocity
